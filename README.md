...
Not all Fitbit web API endpoints are retrieved and not all of the JSON response data is being loaded and visualized. 

Transformation of the data occurs using the Python code before it is loaded to MongoDB, by the Node API server javascript code, or by the HTML page Javascript.

# Use Case

These visualizations augment what Fitbit provides in their app. 

Fitbit visualizations almost always contain only summarized data. For example, the app shows only a single average SpO2 value for a night's sleep. 
...
This app includes the visualizations listed in the home page menu screenshot below. There is also a page that provides a snapshot of today's data and another page that shows detailed sleep activity.
