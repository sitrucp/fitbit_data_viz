...
class FitbitAuthenticator:
    ...
    def get_client(self):
        ...
        if tokens:
            return fitbit.Fitbit(self.client_id, self.client_secret, oauth2=True, access_token=tokens['access_token'], refresh_token=tokens['refresh_token'])
        else:
...
def make_request(start_date, end_date, base_url, module_str, endpoint_type, max_days=None):
    ...
    if not tokens:
        logging.error("AUTHENTICATION ERROR: Token file not found or tokens are invalid. Cannot proceed with requests.")
        return
...
def perform_request(url, date_str, module_str, tokens, attempt=1):
    ...
    try:
        ...
        # Token might be expired
            ...
            logging.error(f"Request failed: {filename} Status code: {response.status_code} - Rate Limit Remaining: {response.headers.get('fitbit-rate-limit-remaining')} - Rate Limit Reset: {response.headers.get('fitbit-rate-limit-reset')}")
